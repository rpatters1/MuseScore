...
bool EnigmaXmlImporter::processBeams(EntryInfoPtr entryInfoPtr, track_idx_t curTrackIdx, std::unordered_map<size_t, ChordRest*>& entryMap)
{
...
void EnigmaXmlImporter::importEntries()
{
    ...
    for (const std::shared_ptr<others::InstrumentUsed>& musxScrollViewItem : musxScrollView) {
        ...
        for (const std::shared_ptr<others::Measure>& musxMeasure : musxMeasures) {
            ...
            logger()->logInfo(String(u"Fixing corruptions for measure at staff %1, tick %2").arg(int(curStaffIdx), currTick.toString()));
